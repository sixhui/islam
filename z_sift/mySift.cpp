...
/**
 * 根据输入图像大小计算高斯金字塔的组数
 * log2{min(M,N)}-t, t in [0, log2{min(M',N')}], M,N 为原始图像尺寸, M',N' 为塔顶图像尺寸
 * @param image 输入原始图像
 * @return
 */
...
/**
 * 使用高斯模糊创建高斯金字塔第一组，第一层图像。第一张图像之所以单独生成，因为可以配置是否升采样，后续的图像都可以基于第一张图像生成。
 *
 * @param image 输入原始图像
 * @param init_image 生成高斯金字塔第一层图像
 */
...
/**
 * 基于最底层的第一张图像，生成高斯金字塔
 * @param init_image 输入已经生成的高斯金字塔第一层图像
 * @param gauss_pyramid 生成高斯金字塔
 * @param nOctaves 输入高斯金字塔的组数
 */
...
/**
 * 精确定位特征点位置(x,y,scale)，用于后面特征点的检测
 * @param dog_pyr
 * @param kpt
 * @param octave
 * @param layer
...
//还可以考虑直接对方向直方图进行细化
/**
 * 在DOG金字塔上进行特征点检测，特征点精确定位，删除低对比度点，删除边缘响应较大点
 *
 * @param dog_pyr
 * @param gauss_pyr
 * @param keypoints
 ...
 *
 * @param image 输入原始图像
 * @param gauss_pyr 输入原始图像的高斯金字塔
 * @param dog_pyr 输入原始图像的高斯差分金字塔
 * @param keypoints 输出检测到的特征点
 * @param all_cell_contrasts
 * @param average_contrast
 * @param n_cells
 * @param num_cell
 * @param available_n
 * @param available_num
 * @param final_keypoints
 * @param Final_keypoints
 * @param Final_Keypoints
 */
