 ...
 * @param H 单应变换矩阵
 * @param x1 输入，世界坐标
 * @param x2 输出，观测值，像素坐标
 * @param forward_error
 * @param backward_error
 */
template <typename T>
...
public:
    ...
    /**
     * BA 优化，优化的误差项为 Hx=y
     * 输入单幅图像的特征点像素坐标和对应的世界坐标，再输入未优化的 H 矩阵
     * 输出该图像计算得到的优化后的 H 矩阵
     *
     * ceres优化的几个要点： 优化变量的维数、输入输出与观测值、误差公式
     *
     * @param img_pts_
     * @param board_pts_
     * @param matrix_H_
	...
	{
		Eigen::MatrixXd x1(2, board_pts_.size());   // 二维世界坐标 - 输入
		Eigen::MatrixXd x2(2, img_pts_.size());     // 二维像素坐标 - 输出
		...
		for (int i = 0; i < x1.cols(); i++){
			...
			problem.AddResidualBlock(
				...
				HomographySymmetricGeometricCostFunctor,
				4,  // num_residuals - 一个H两个等式，并使用双向误差
				9>(homography_symmetric_geometric_cost_function),
				NULL,
				H.data());
//            std::cout << H << std::endl;
//            std::cout << "---" << std::endl;
//            std::cout << *(H.data() + 4) << std::endl;
//            exit(0);
		}
    ...
    /**
     *
     * @param params_ 待优化参数的初值
     * @param imgs_pts_
     * @param bords_pts_
     * @param refined_params_ 输出优化后的参数
     */
		...
		Params &refined_params_)
	{
		...
		{
//            std::cout << vec_rt[n].block<3, 3>(0, 0) << std::endl;
//            std::cout << vec_rt[n] << std::endl;
//            exit(1);
...
private:
		...
		Params &params_)
	{
		...
		for (const auto &rt:v_rt_)
		{
			...
			t.block<3, 3>(0, 0) = r_v.toRotationMatrix();
			t.block<3, 1>(0, 3) = Eigen::Vector3d(rt(3), rt(4), rt(5));
			params_.vec_rt.push_back(t);
		}
	}
    ...
    /**
     * 两维残差
     * 私有成员：输入、观测数据，并由构造函数传入
     * bool operator()(优化变量，残差)const{}
     */
	struct HomographySymmetricGeometricCostFunctor
	{
		...
		const Eigen::Vector2d x_;   // 输入数据 - 世界坐标
		const Eigen::Vector2d y_;   // 观测数据 - 像素坐标
	};

    /**
     * 损失函数：经过全部变换，从世界坐标到像素坐标
     * ！！！涉及对旋转的求导，需要进行李代数变换
     */
